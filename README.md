...
- The automated Level 1 tester has five selectable skill profiles: `Best`, `Skilled`, `Average`, `Casual`, and `Novice`. Every profile spends all currently spendable currency; weaker profiles make progressively less efficient upgrade and placement choices and deliberately devote some budget to defensive/utility ranks. `Skilled`, `Average`, and `Casual` still use the active weapon regularly, while only `Novice` has strongly reduced active-weapon timing. `Best` continues to avoid base-life ranks so it remains the offensive benchmark.
